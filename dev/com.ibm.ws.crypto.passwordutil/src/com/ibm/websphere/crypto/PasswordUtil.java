            ...
            if (encoded_string.length() > 0) {
                ...
                if (encrypted_bytes.length > 0) {
                    ...
                    try {
                        ...
                        System.out.println("Time (milliseconds) before decipher: " + beforeTime);
                        decrypted_bytes = PasswordCipherUtil.decipher(encrypted_bytes, crypto_algorithm);
                        long afterTime = Instant.now().toEpochMilli();
                        System.out.println("Time (milliseconds) after decipher: " + afterTime);
                        System.out.println("Time (milliseconds) elapsed decipher: " + (afterTime - beforeTime));
            ...
            if (decoded_string.length() > 0) {
                ...
                if (decrypted_bytes.length > 0) {
                    ...
                    while (!done) {
                        try {
                            long beforeTime = Instant.now().toEpochMilli();
                            System.out.println("Time (milliseconds) before encipher_internal: " + beforeTime);
                            ...
                            long afterTime = Instant.now().toEpochMilli();
                            System.out.println("Time (milliseconds) after encipher_internal: " + afterTime);
                            System.out.println("Time (milliseconds) elapsed encipher_internal: " + (afterTime - beforeTime));
