...
final class LTPACrypto {
    ...
    protected static final byte[] signISO9796(byte[][] key, byte[] data, int off, int len) throws Exception {
        ...
        // TODO prototype - caching disabled
        /*
         * CachingKey ck = new CachingKey(key, data, off, len); CachingKey result =
         * cryptoKeysMap.get(ck);
         *
         * if (result != null) { result.successfulUses += 1; result.reused = true;
         * return result.result; } else { if (cryptoKeysMap.size() > MAX_CACHE) { try {
         * CachingKey[] keys = cryptoKeysMap.keySet().toArray(new
         * CachingKey[cryptoKeysMap.size()]); Arrays.sort(keys, cachingKeyComparator);
         * if (cachingKeyComparator.compare(keys[0], keys[keys.length - 1]) < 0) { for
         * (int i = 0; i < cryptoKeysMap.size() / 5; i++) {
         * cryptoKeysMap.remove(keys[i]); keys[i + 1 * cryptoKeysMap.size() /
         * 5].successfulUses--; keys[i + 2 * cryptoKeysMap.size() / 5].successfulUses--;
         * keys[i + 3 * cryptoKeysMap.size() / 5].successfulUses--; keys[i + 4 *
         * cryptoKeysMap.size() / 5].successfulUses--; } } else { for (int i = 0; i <
         * cryptoKeysMap.size() / 5; i++) { cryptoKeysMap.remove(keys[keys.length - 1 -
         * i]); keys[keys.length - 1 - i - 1 * cryptoKeysMap.size() /
         * 5].successfulUses--; keys[keys.length - 1 - i - 2 * cryptoKeysMap.size() /
         * 5].successfulUses--; keys[keys.length - 1 - i - 3 * cryptoKeysMap.size() /
         * 5].successfulUses--; keys[keys.length - 1 - i - 4 * cryptoKeysMap.size() /
         * 5].successfulUses--; } } } catch (Exception e) { // do nothing. since this
         * code is used for the command line // utility, no log is // taken. }
         *
         * } }
         */

        /**
         * Invoked by LTPADigSignature * BigInteger n = new BigInteger(key[0]);
         * BigInteger e = new BigInteger(key[2]); BigInteger p = new BigInteger(key[3]);
         * BigInteger q = new BigInteger(key[4]); System.out.println("REMOVED modInverse
         * len: " + len); // BigInteger d = //
         * e.modInverse((p.subtract(BigInteger.ONE)).multiply(q.subtract(BigInteger.ONE)));
         * BigInteger d = new BigInteger(key[1]); // JOHN TRY
         * System.out.println("private exponent: " + d); System.out.println("AFTER
         * modInverse"); KeyFactory kFact = null;
         *
         * kFact = (provider == null) ? KeyFactory.getInstance(CRYPTO_ALGORITHM_RSA) :
         * KeyFactory.getInstance(CRYPTO_ALGORITHM_RSA, provider);
         *
         * BigInteger pep = new BigInteger(key[5]); BigInteger peq = new
         * BigInteger(key[6]); BigInteger crtC = new BigInteger(key[7]);
         * RSAPrivateCrtKeySpec privCrtKeySpec = new RSAPrivateCrtKeySpec(n, e, d, p, q,
         * pep, peq, crtC); PrivateKey privKey = kFact.generatePrivate(privCrtKeySpec);
         */
        ...
        System.out.println("using RSAprivKey object"); // TODO prototype
    ...
    protected static final boolean verifyISO9796(byte[][] key, byte[] data, int off, int len, byte[] sig, int sigOff,
        ...
        // TODO prototype - removed caching
        /*
         * CachingVerifyKey ck = new CachingVerifyKey(key, data, off, len, sig, sigOff,
         * sigLen); CachingVerifyKey result = verifyKeysMap.get(ck);
         *
         * if (result != null) { result.successfulUses += 1; return result.result; }
         * else { if (verifyKeysMap.size() > MAX_CACHE) { CachingVerifyKey[] keys =
         * verifyKeysMap.keySet().toArray(new CachingVerifyKey[verifyKeysMap.size()]);
         * Arrays.sort(keys, cachingVerifyKeyComparator); if
         * (cachingVerifyKeyComparator.compare(keys[0], keys[keys.length - 1]) < 0) {
         * for (int i = 0; i < verifyKeysMap.size() / 5; i++) {
         * verifyKeysMap.remove(keys[i]); keys[i + 1 * verifyKeysMap.size() /
         * 5].successfulUses--; keys[i + 2 * verifyKeysMap.size() / 5].successfulUses--;
         * keys[i + 3 * verifyKeysMap.size() / 5].successfulUses--; keys[i + 4 *
         * verifyKeysMap.size() / 5].successfulUses--; } } else { for (int i = 0; i <
         * verifyKeysMap.size() / 5; i++) { verifyKeysMap.remove(keys[keys.length - 1 -
         * i]); keys[keys.length - 1 - i - 1 * verifyKeysMap.size() /
         * 5].successfulUses--; keys[keys.length - 1 - i - 2 * verifyKeysMap.size() /
         * 5].successfulUses--; keys[keys.length - 1 - i - 3 * verifyKeysMap.size() /
         * 5].successfulUses--; keys[keys.length - 1 - i - 4 * verifyKeysMap.size() /
         * 5].successfulUses--; } } } }
         */
    ...
    // TODO prototype
    protected static final void setPrivateKey(RSAPrivateCrtKey key) {
        rsaPrivKey = key;
    }

    // TODO prototype
    protected static final void setPublicKey(RSAPublicKey key) {
        rsaPubKey = key;
    }
